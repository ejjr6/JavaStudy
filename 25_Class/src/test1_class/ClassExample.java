...
// 참고 - https://mirotic91.tistory.com/16
public class ClassExample {
	...
	public static void main(String[] args) throws ClassNotFoundException {
		...
		try {
			...
			Person p1 = 
				(Person)pClass3.getConstructor(String.class).newInstance("우리집");
			System.out.println(p1);
			Person p2 = 
				(Person)pClass3.getConstructor(String.class, int.class).newInstance("남의집",30);
			System.out.println(p2);
		} catch (InstantiationException e) {
			// 객체 설계 정보는 있으나 객체 생성이 불가능 할때 발생하는 오류
			// 추상클래스, 인터페이스 이러한 설계정보를 가진 class는
			// 자체만으로 객체 생성이 불가하니 생성 할 수 없다고 발생하는 예외
			e.printStackTrace();
